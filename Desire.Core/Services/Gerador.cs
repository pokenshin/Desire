...
namespace Desire.Core.Services
{
    public class Gerador
    {
        ...
        string[] tiposHabilidade = new string[] { "Melhora", "Piora", "Danifica", "Cura", "Fugacidade", "Deslocamento" };
        ...
        public Ser GeraSerAleatorio()
        {
            ...
            Ser ser = new Ser()
            {
                ...
                //Perícias
                Pericias = GeraListaPericias(),
                //Itens e Equips
                Posses = GeraListaItens(GeraInteiro(10, 30)),
                //Origem do Poder
                OrigemPoder = GeraOrigemPoder(),
                //Virtudes
                Virtudes = GeraListaModificadores("Virtudes", 3, '+'),
                //Defeitos
                Defeitos = GeraListaModificadores("Defeitos", 6, '-'),
                //Resistências
                Resistencias = GeraListaResistencias(),
                //Gênese
                Genese = GeraInteiro(1, 5),
                //Geração
                Geracao = GeraInteiro(1, 10),
                //Trajetória
                ...
                SubatributoExtraNome1 = GeraNomeAleatorio(3, 10),
                SubatributoExtraValor1 = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 10)),
                SubatributoExtraNome2 = GeraNomeAleatorio(3, 10),
                SubatributoExtraValor2 = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 10))
            };
            //Itens Equipados
            ser.Equipamentos = GeraListaEquipamentos(ser.Especies[0].MaxItensEquipados, ser.Especies[0].MaxArmasEquipadas);
        ...
        public List<Resistencia> GeraListaResistencias(int numero = 0)
        {
            List<Resistencia> resultado = new List<Resistencia>();

            if (numero == 0)
                numero = GeraInteiro(2, 10);

            for (int i = 0; i < numero; i++)
            {
                resultado.Add(GeraResistencia());
            }

            return resultado;
        }

        public Resistencia GeraResistencia(char tipo = 'R')
        {
            CalculadorNumero calculador = new CalculadorNumero();
            Resistencia resultado = new Resistencia();
            ValorMag min = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 10));
            ValorMag max = calculador.SomaValorMag(min, new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 5)));
            ...
            resultado.Nome = GeraNomeAleatorio(3, 10);

            if (tipo == '+')
            {
                resultado.Positiva = GeraValorMag(min, max);
                resultado.Negativa = new ValorMag();
            }
            else
            { 
                resultado.Positiva = new ValorMag();
                resultado.Negativa = GeraValorMag(min, max);
            }

            return resultado;
        }

        public string GeraOrigemPoder()
        {
            //TODO: Gerar lista coerente de poderes a partir de um banco de dados
            return "Origem Aleatória de poder";
        }
        ...
        public Especie GeraEspecie()
        {
            ...
            Especie especie = new Especie()
            {
                ...
                Natureza = GeraNatureza(),
                RespostaMin = GeraResposta()
            };
            ...
            especie.IraMax = calculadorNum.SomaValorMag(especie.IraMin, especie.IraMin);
            ...
            especie.MaturidadeMin = (int)calculadorNum.CalculaPorcentagem(GeraInteiro(1, 30), especie.TempoMax);
            especie.MaturidadeMax = especie.MaturidadeMin + (int)calculadorNum.CalculaPorcentagem(GeraInteiro(60, 99), especie.TempoMax);
            especie.TrabalhoMax = especie.TrabalhoMin + GeraLong(0, 1000);
            especie.LarguraMax = calculadorNum.SomaValorMag(especie.LarguraMin, new ValorMag(Convert.ToString(GeraLong(1, 100))));
            especie.RespostaMax = new Resposta()
            {
                Bravura = especie.RespostaMin.Bravura + GeraInteiro(1, 1000),
                Coragem = especie.RespostaMin.Coragem + GeraInteiro(1, 1000),
                Desespero = especie.RespostaMin.Desespero + GeraInteiro(1, 1000),
                Heroismo = especie.RespostaMin.Heroismo + GeraInteiro(1, 1000),
                Indiferenca = especie.RespostaMin.Indiferenca + GeraInteiro(1, 1000),
                Medo = especie.RespostaMin.Medo + GeraInteiro(1, 1000),
                Panico = especie.RespostaMin.Panico + GeraInteiro(1, 1000)
            };
            return especie;
        }

        public Resposta GeraResposta()
        {
            Resposta resposta = new Resposta()
            {
                Bravura = GeraInteiro(1, 1000),
                Coragem = GeraInteiro(1, 1000),
                Desespero = GeraInteiro(1, 1000),
                Heroismo = GeraInteiro(1, 1000),
                Indiferenca = GeraInteiro(1, 1000),
                Medo = GeraInteiro(1, 1000),
                Panico = GeraInteiro(1, 1000)
            };

            return resposta;
        }

        public Natureza GeraNatureza()
        {
            Natureza resultado = new Natureza()
            {
                Apresentacao = GeraInteiro(1, 26),
                Concepcao = GeraInteiro(1, 26),
                Honra = GeraInteiro(1, 26),
                Moral = GeraInteiro(1, 26),
                Percepcao = GeraInteiro(1, 26),
                Personalidade = GeraInteiro(1, 26)
            };
        ...
        public Habilidade GeraHabilidade()
        {
            ...
            {
                Id = GeraInteiro(1, 1000),
                Magnitude = GeraInteiro(1, 10),
                Nome = GeraNomeAleatorio(2, 8),
                PericiaAssociada = GeraPericia(),
                TipoHabilidade = tiposHabilidade[tipoIndex],
                Energia = GeraEnergia(),
                ValorEfeito = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1,15)),
                Duracao = duracoes[duracaoIndex],
                ValorDuracao = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 5)),
                Area = GeraAreaCientifica()
            };
            habilidade.Caracteristicas = habilidade.TipoHabilidade + " " + habilidade.ValorEfeito + " pontos";
        ...
        public Atributo GeraAtributo(string v = "Random")
        {
            ...
            switch (v)
            {
                case "Força":
                    ...
                    {
                        Sustentacao = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 15)),
                        Classe = Convert.ToChar(GeraNomeAleatorio(1, 1)),
                        Dureza = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 15)),
                        Golpe = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 15)),
                        Nivel = GeraInteiro(1, 6),
                        Pontos = GeraInteiro(1, 100),
                        Porcentagem = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 15)),
                        Potencia = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 15)),
                        Vigor = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 15)),
                        Evolucao = new Evolucao(GeraInteiro(1, 17), 16, GeraBoolean(50)),
                        BonusAP = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 5))
                    };
                    return forca;
                ...
                case "Destreza":
                    Destreza destreza = new Destreza()
                    {
                        ...
                        Dinamica = new ValorMag(GeraInteiro(1, 99), GeraInteiro(-1, 5)),
                ...
                case "Materia":
                    Materia materia = new Materia()
                    {
                        ...
                        Impulso = new ValorMag(GeraInteiro(1, 100), GeraInteiro(1, 15)),
                        Nivel = GeraInteiro(1, 6),
                        Pontos = GeraInteiro(1, 100),
                        Porcentagem = new ValorMag(GeraInteiro(1, 100), GeraInteiro(1, 15)),
                        ...
                        Evolucao = new Evolucao(GeraInteiro(1, 17), 16, GeraBoolean(50)),
                        BonusHP = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 5)),
                        BonusAP = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 5))
                    };
                    return materia;
                ...
                case "Criatividade":
                    Criatividade criatividade = new Criatividade()
                    {
                        ...
                        Evolucao = new Evolucao(GeraInteiro(1, 17), 16, GeraBoolean(50)),
                        BonusMP = new ValorMag(GeraInteiro(1, 99), GeraInteiro(1, 5))
                    };
                    return criatividade;
        ...
        private Energia GeraEnergia()
        {
            ...
            {
                Sigla = tiposEnergia[GeraInteiro(0, tiposEnergia.Count-1)],
